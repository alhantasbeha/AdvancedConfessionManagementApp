...
export const BirthdaysPage: React.FC = () => {
  ...
  const filteredBirthdays = useMemo(() => {
    const today = new Date();
    let result: Confessor[] = [];

    const activePeople = confessors.filter(c => c.birthDate && !c.isDeceased && !c.isArchived);

    if (selectedPeriod === 'اليوم') {
      result = activePeople.filter(p => {
  ...
  }, [confessors, selectedPeriod, selectedMonth]);

  const handleSendMessage = (person: Confessor) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg text-center">
          ...
          <p className="text-2xl font-bold text-blue-700 dark:text-blue-200">
            {filteredBirthdays.filter(p => getDaysUntilBirthday(p.birthDate) === 0).length}
          </p>
        ...
        <div className="bg-green-50 dark:bg-green-900 p-4 rounded-lg text-center">
          ...
          <p className="text-2xl font-bold text-green-700 dark:text-green-200">
            {filteredBirthdays.filter(p => getDaysUntilBirthday(p.birthDate) === 1).length}
          </p>
        ...
        <div className="bg-purple-50 dark:bg-purple-900 p-4 rounded-lg text-center">
          <Icon name="calendar" className="w-8 h-8 mx-auto text-purple-500 mb-2" />
          <p className="text-2xl font-bold text-purple-700 dark:text-purple-200">
            {filteredBirthdays.filter(p => {
      ...
      <div className="space-y-4">
        {filteredBirthdays.length === 0 ? (
          ...
          filteredBirthdays.map(person => {
            ...
            return (
              <div key={person.id} className="bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900 dark:to-purple-900 p-4 rounded-lg border border-pink-200 dark:border-pink-700">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    {person.phone1 && person.phone1Whatsapp && (
                      <button 
