...
export const ConfessionLogPage: React.FC = () => {
  ...
  useEffect(() => {
    let results = logs.filter(log => {
      ...
      const matchesSearch = searchTerm === '' || confessorName.includes(searchTerm.toLowerCase());
      ...
      return matchesSearch && matchesConfessor && matchesDateFrom && matchesDateTo;
    });
    
    results.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    setFilteredLogs(results);
  }, [logs, searchTerm, selectedConfessor, dateFrom, dateTo, getConfessorNameById]);
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
          ...
          <p className="text-2xl font-bold text-blue-700 dark:text-blue-200">{filteredLogs.length}</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-right">
          <thead>
            <tr className="border-b dark:border-gray-700">
              <th className="p-3 text-right">المعترف</th>
              <th className="p-3 text-right">تاريخ الاعتراف</th>
        ...
        {filteredLogs.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">لا توجد سجلات اعتراف تطابق البحث</p>
          </div>
        )}
      </div>
